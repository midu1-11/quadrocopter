...
{	
    ...
    float Pitch[4];
		long quat[4];
				while(dmp_read_fifo(gyro, accel, quat, &sensor_timestamp, &sensors, &more)!=0){}		//读取DMP数据
				...
				{    
					 ...
					 q2=quat[2] / q30;
					 q3=quat[3] / q30;  //四元数
					 Pitch[0] = (int)(atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3); // 计算出俯仰角
					 Pitch[1] = (int)(asin(2*(q0*q2-q3*q1))* 57.3); // 计算出翻滚角
					 Pitch[2] = (int)(atan2(2 * q0 * q3 + 2 * q2 * q1, -2 * q3 * q3 - 2 * q2* q2 + 1)* 57.3); // 计算出偏航角
					 Pitch[3] = accel[2];
				}
		return Pitch;
}
